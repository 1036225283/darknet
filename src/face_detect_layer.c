...
box get_face_detect_box(float *x, float *biases, int n, int index, int i, int j, int w, int h, int stride)
{
    box b;
...
float delta_face_detect_box(box truth, float *x, float *biases, int n, int index, int i, int j, int w, int h, float *delta, float scale, int stride)
{
    box pred = get_face_detect_box(x, biases, n, index, i, j, w, h, stride);
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    box pred = get_face_detect_box(l.output, l.biases, n, box_index, i, j, l.w, l.h, l.w*l.h);
                    float best_iou = 0;
                    for(t = 0; t < 30; ++t){
        ...
        for(t = 0; t < 30; ++t){
            ...
            float iou = delta_face_detect_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, l.delta, l.coord_scale *  (2 - truth.w*truth.h), l.w*l.h);
            if(iou > .5) recall += 1;
            avg_iou += iou;
...
void get_face_detect_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, float tree_thresh, int relative, detection *dets)
{
    int i,j,n,z;
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            dets[index].bbox = get_face_detect_box(predictions, l.biases, n, box_index, col, row, l.w, l.h, l.w*l.h);
    ...
    correct_face_detect_boxes(dets, l.w*l.h*l.n, w, h, netw, neth, relative);
}
